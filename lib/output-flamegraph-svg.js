...
function FlameGraphEmitter(args)
{
	...
	this.collapsed = args.collapsed === undefined ? true : args.collapsed;

	this.fge_params = {
	    'nojs': args.nojs || false,
...
FlameGraphEmitter.prototype.run = function (callback)
{
	...
	var iterator = this.collapsed ? this.fge_stacks.eachStackByStack : this.fge_stacks.eachStack;
	iterator.call(this.fge_stacks, flow);
	flow([], 0);
...
FlameGraphEmitter.prototype.draw = function (callback)
{
	...
	if (!this.fge_params.nojs) {
		xml.emitStart('script', { 'type': 'text/ecmascript' });
		var script = mod_fs.readFileSync(__dirname + '/../share/flamegraph-svg.js', 'utf8');
		xml.emitCDataSection(script);
		xml.emitEnd('script');
	}
	...
	emitString(xml, black, fontface, fontsize, width - xpad - 100,
	...
	emitString(xml, black, fontface, fontsize, width - xpad - 100, height - (ypadbtm / 2), ' ', '', { 'id': 'matched' });
