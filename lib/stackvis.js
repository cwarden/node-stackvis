...
function StackSet()
{
	...
	this.ss_orderedStacks = []; /* stacks, in the order they were emitted */
}
...
StackSet.prototype.addStack = function (stack, count)
{
	...
	this.ss_orderedStacks.push({ stack: stack, count: count});
};
...
/*
 * Iterates all stacks in order added
 */
StackSet.prototype.eachStack = function (callback)
{
	this.ss_orderedStacks.forEach(function (stack) {
		callback(stack.stack, stack.count);
	});
};
